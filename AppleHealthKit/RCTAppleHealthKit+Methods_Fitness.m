...
- (void)fitness_getDailyStepSamples:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    [self fetchCumulativeSumStatisticsCollectionWithPredicateDiscrete:stepCountType
                                            unit:unit
                                       startDate:startDate
                                         endDate:endDate
                                       ascending:ascending
...
- (void)fitness_getFlightsClimbedOnDay:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    ...
    NSDate *startDate = [RCTAppleHealthKit dateFromOptions:input key:@"startDate" withDefault:nil];
    NSDate *endDate = [RCTAppleHealthKit dateFromOptions:input key:@"endDate" withDefault:[NSDate date]];
    HKUnit *cal = [RCTAppleHealthKit hkUnitFromOptions:input key:@"unit" withDefault:[HKUnit countUnit]];
    ...
    [self fetchQuantitySamplesOfType:quantityType
                          ...
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  callback(@[RCTMakeError(@"error getting FlightsClimbed", nil, nil)]);
                                  return;
                              }
                          }];
}

- (void)fitness_getDistanceWheelchair:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    [self fetchSumOfSamplesOnDayForType:quantityType unit:unit day:date completion:^(double count, NSDate *startDate, NSDate *endDate, NSError *error) {
        if (!count) {
            NSLog(@"ERROR getting DistanceWheelchair: %@", error);
            callback(@[RCTMakeError(@"ERROR getting DistanceWheelchair", error, nil), @(count)]);
            return;
        }
...
- (void)fitness_getSwimmingStrokeCount:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    HKUnit *unit = [RCTAppleHealthKit hkUnitFromOptions:input key:@"unit" withDefault:[HKUnit mileUnit]];
    NSDate *date = [RCTAppleHealthKit dateFromOptions:input key:@"date" withDefault:[NSDate date]];
...
- (void)fitness_getDistanceSwimming:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    [self fetchSumOfSamplesOnDayForType:quantityType unit:unit day:date completion:^(double count, NSDate *startDate, NSDate *endDate, NSError *error) {
        if (!count) {
            NSLog(@"ERROR getting Distance Swimming: %@", error);
            callback(@[RCTMakeError(@"ERROR getting Distance Swimming", error, nil), @(count)]);
            return;
        }
...
- (void)fitness_getBasalEnergyBurned:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    [self fetchQuantitySamplesOfType:basalEnergyType
                          ...
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  callback(@[RCTMakeError(@"error getting BasalEnergyBurned", nil, nil)]);
                                  return;
                              }
                          }];
}

- (void)fitness_getNikeFuel:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    HKUnit *unit = [HKUnit countUnit];
    NSDate *date = [RCTAppleHealthKit dateFromOptions:input key:@"date" withDefault:[NSDate date]];
...
- (void)fitness_getExerciseTime:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    ...
    NSDate *startDate = [RCTAppleHealthKit dateFromOptions:input key:@"startDate" withDefault:nil];
    NSDate *endDate = [RCTAppleHealthKit dateFromOptions:input key:@"endDate" withDefault:[NSDate date]];
    HKUnit *cal = [RCTAppleHealthKit hkUnitFromOptions:input key:@"unit" withDefault:[HKUnit minuteUnit]];
    ...
    [self fetchQuantitySamplesOfType:quantityType
                          ...
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting getExerciseTime: %@", error);
                                  callback(@[RCTMakeError(@"error getting getExerciseTime", nil, nil)]);
                                  return;
                              }
                          }];
}

- (void)fitness_getStandHour:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback {
    HKUnit *unit = [HKUnit countUnit];
    NSDate *date = [RCTAppleHealthKit dateFromOptions:input key:@"date" withDefault:[NSDate date]];

    HKQuantityType *quantityType = [HKObjectType quantityTypeForIdentifier:HKCategoryTypeIdentifierAppleStandHour];

    [self fetchSumOfSamplesOnDayForType:quantityType unit:unit day:date completion:^(double count, NSDate *startDate, NSDate *endDate, NSError *error) {
        if (!count) {
            NSLog(@"ERROR getting getStandHour: %@", error);
            callback(@[RCTMakeError(@"ERROR getting getStandHour", error, nil), @(count)]);
            return;
        }
